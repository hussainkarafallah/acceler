                              ...
                              Number *destination)
{
  const int idx = threadIdx.x + blockIdx.x * blockDim.x;
  if (idx < N)
    destination[idx] = scalar2 * destination[idx] + scalar1 * source[idx];
}
...
#pragma omp parallel for simd
  ...
  void sadd(const Number my_scalar, const Number other_scalar, const Vector &other)
  {
    assert_size(other);
    ...
    if (memory_space == MemorySpace::CUDA)
    {
      ...
      vector_update<<<n_blocks, block_size>>>(local_size, other_scalar, my_scalar, other.data, data);
#endif
    }
    else
    {
